...
const downloadDashboardBtn = document.getElementById("download-dashboard");
...
function normalizeRecord(row) {
  ...
  return {
    year: toText(row.year ?? row.Year1),
...
async function exportElementAsPng(element, baseName) {
  ...
  link.href = dataUrl;
  link.download = `${slugify(baseName)}-${timestampLabel()}.png`;
...
function renderScatter(records) {
  ...
  for (let tick = 1; tick <= 9; tick += 1) {
    const x = scaleX(tick);
    const tickLineX = document.createElementNS(svg.namespaceURI, "line");
    tickLineX.setAttribute("x1", x);
    tickLineX.setAttribute("y1", height - margin.bottom);
    tickLineX.setAttribute("x2", x);
    tickLineX.setAttribute("y2", height - margin.bottom + 6);
    tickLineX.setAttribute("stroke", "#d1c7bb");
    svg.appendChild(tickLineX);

    const labelX = document.createElementNS(svg.namespaceURI, "text");
    labelX.setAttribute("x", x);
    labelX.setAttribute("y", height - margin.bottom + 18);
    labelX.setAttribute("text-anchor", "middle");
    labelX.setAttribute("font-size", "10");
    labelX.setAttribute("fill", "#6b7280");
    labelX.textContent = String(tick);
...
async function handleRun() {
  ...
  if (downloadDashboardBtn) downloadDashboardBtn.disabled = true;
  downloadHint.textContent = "Working…";
  setProgress(0, 1);

  try {
    const { workbook, previews, summary, records } = await convertPdfsToWorkbook(state.files, (current, total) => {
    ...
    if (downloadDashboardBtn) downloadDashboardBtn.disabled = false;
...
if (downloadDashboardBtn) downloadDashboardBtn.addEventListener("click", handleDownload);
if (dashboardRefreshBtn) dashboardRefreshBtn.addEventListener("click", () => renderDashboard(state.records));
refreshBtn.addEventListener("click", () => {
  const sheet = sheetSelect.value;
  if (sheet) renderPreview(sheet);
  if (state.records.length) renderDashboard(state.records);
});
